...
async function checkEdgeCreate(that, edge) {
...
async function checkEdgeModify(edge) {
  if (!validateEdge(edge)) {
    throw new Error('Edge contains invalid or missing properties');
  }
  if (!viewer.isWriteAll()) {
...
class DB {
  ...
  static async createObject(viewer, type, data) {
    ...
    return (!error && (result.affectedRows == 1)) ? uuid : null;
  }
  ...
  static async createEdge(viewer, edge) {
    var reverse_edge_type = Constants.Edges[edge.getType()].reverse_edge;
    if (Number.isInteger(reverse_edge_type) && reverse_edge_type >= 0) {
      ...
      await Promise.all([
        checkEdgeCreate(this, edge),
        checkEdgeCreate(this, r_edge)
      ]);
      var [result, error] = await this.execute(
      ...
      await checkEdgeCreate(this, edge);
      var [result, error] = await this.execute(
  ...
  static async modifyEdgeData(viewer, edge) {
    ...
    if (Number.isInteger(reverse_edge_type) && reverse_edge_type >= 0) {
      ...
      await Promise.all([
        checkEdgeModify(edge),
        checkEdgeModify(r_edge)
      ]);
      var [result, error] = await this.execute(
