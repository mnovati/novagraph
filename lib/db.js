...
class DB {
  ...
  async getObject(viewer, id) {
    ...
    var object = this.Constants.getObjectInstance(wiewer, result).withDB(this);
    if (!viewer.isReadAll()) {
  ...
  async createObject(viewer, type, data) {
    ...
    var temp_object = this.Constants.getObjectInstance(viewer, { id: uuid, type: type, data: data }).withDB(DB);
    if (!viewer.isWriteAll()) {
      ...
      temp_object = this.Constants.getObjectInstance(viewer, { id: uuid, type: type, data: data }).withDB(DB);
    }
