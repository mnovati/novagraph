...
async function checkObjectTypes(DB, viewer, id, type, data) {
  ...
  for (var type in types) {
    if (type in data) {
      var result = await types[type].check(DB, viewer, data[type]);
      if (!result) {
...
async function checkEdgeCreate(Constants, that, viewer, edge) {
  ...
  if (!viewer.isWriteAll()) {
    var can_create = await edge.canCreate(this);
    if (!can_create) {
...
async function checkEdgeModify(Constants, viewer, edge) {
  ...
  if (!viewer.isWriteAll()) {
    var can_modify = await edge.canModify(this);
    if (!can_modify) {
...
class DB {
  ...
  async getObject(viewer, id) {
    ...
    var object = this.Constants.getObjectInstance(viewer, result);
    if (!viewer.isReadAll()) {
      var can_see = await object.canSee(this);
      if (can_see) {
        var data = Object.assign({}, result.data);
        await Promise.all(Object.keys(data).map(async key => {
          var can = await object.canSeeField(this, key);
          if (!can) {
            delete data[key];
          }
        }));
        result.data = data;
        object = this.Constants.getObjectInstance(viewer, result);
      } else {
        object = null;
      }
    }
  ...
  async createObject(viewer, type, data) {
    ...
    var temp_object = this.Constants.getObjectInstance(viewer, { id: uuid, type: type, data: data });
    if (!viewer.isWriteAll()) {
      var can_create = await temp_object.canCreate(this);
      if (!can_create) {
        throw NError.normal('Viewer does not have permission to create object');
      }
      await Promise.all(Object.keys(data).map(async key => {
        var can = await temp_object.canCreateField(this, key);
        if (!can) {
          delete data[key];
        }
      }));
      temp_object = this.Constants.getObjectInstance(viewer, { id: uuid, type: type, data: data });
    }
    ...
    if (root_id) {
      ...
      await this.createEdge(
        write_all,
        this.Constants.getEdgeInstance(write_all, {from_id: root_id, to_id: uuid, type: this.Constants.ROOT_EDGE})
      );
    }
  ...
  async modifyObjectData(viewer, id, data_in) {
    ...
    if (!viewer.isWriteAll()) {
      var can_modify = await old_object.canModify(this);
      if (!can_modify) {
    ...
    if (!viewer.isWriteAll()) {
      await Promise.all(Object.keys(data).map(async key => {
        var can = await old_object.canModifyField(this, key);
        if (!can) {
          delete data[key];
        }
      }));
  ...
  async setObjectStatus(viewer, object, new_status) {
    ...
    if (!viewer.isWriteAll()) {
      var can_modify = await object.canModify(this);
      if (!can_modify) {
  ...
  async getSingleEdge(viewer, from_id, type, to_id) {
    ...
    result.type = parseInt(result.type);
    var edge = this.Constants.getEdgeInstance(viewer, result);
    if (!viewer.isReadAll()) {
      var can_see = await edge.canSee(this);
  ...
  async getEdge(viewer, id, type) {
    ...
    for(var ii = 0; ii < results.length; ii++) {
      ...
      handles.push((async () => {
        var edge = this.Constants.getEdgeInstance(viewer, results[ii]);
        if (viewer.isReadAll()) {
          return edge;
        }
        var can_see = await edge.canSee(this);
