...
class DB {
  ...
  static async modifyObject(viewer, object) {
    ...
    if (geo_index) {
      for (var key in data) {
        if (geo_index === key) {
          await this.deleteGeoIndex(temp_object);
          await this.insertGeoIndex(temp_object, key);
        }
      }
    }
  ...
  static async lookupGeoIndex(point, types, distance) {
    ...
    var [rows, _] = await this.execute(
      'SELECT bin2uuid(`id`) as id, round(st_distance_sphere(shape, st_geomfromtext(?, 1)), 2) as dist '+
      'FROM geoindices '+
      'WHERE `type` in (?) AND dist <= ?;',
      [value, types, distance]
    );
    if (!rows) {
      return null;
    }
