...
function checkObjectTypes(object) {
  var config = Constants.getObject(object.getType());
  var types = config.types || {};
  var data = object.object.data;
  for (var type in types) {
    if (type in data) {
      if (!types[type].check(data[type])) {
        throw NError.normal('Object invalid type for field', { id: object.getID(), field: type });
      }
    }
  }
}
...
async function checkEdgeCreate(that, viewer, edge) {
  ...
  if (edge.getType() === Constants.COGNITO_EDGE) {
    ...
    if (!objects[0] || (config.from_type.length > 0 && !config.from_type.includes(objects[0].getType()))) {
      throw NError.normal('Invalid from type, or from object cannot be loaded for edge', { type: edge.getType() });
    }
    if (!objects[1] || (config.to_type.length > 0 && !config.to_type.includes(objects[1].getType()))) {
      throw NError.normal('Invalid to type, or to object cannot be loaded for edge', { type: edge.getType() });
    }
  }
...
class DB {
  ...
  static async getObject(viewer, id) {
    ...
    if (!viewer.isReadAll()) {
      ...
      if (can_see) {
        ...
        object = Constants.getObjectInstance(viewer, { id: object.getID(), type: object.getType(), data: data });
      } else {
        object = null;
      }
    }
  ...
  static async createObject(viewer, type, data) {
    ...
    if (!viewer.isWriteAll()) {
      ...
      await Promise.all(Object.keys(data).map(async key => {
        var can = await temp_object.canModifyField(key);
        if (!can) {
          delete data[key];
        }
      }));
    ...
    await Promise.all((Constants.getObject(type).unique_index || []).map(async index => {
      if (index in data) {
        ...
        if (existing && existing.length > 0) {
          throw NError.normal('Duplicate index entry attempted', { id: uuid, type: type, field: index });
        }
      }
    }));
  ...
  static async modifyObject(viewer, object) {
    ...
    var old_data = old_object.object.data;
      ...
      if (index in data) {
        if (stringifyObjectValue(data[index]) !== stringifyObjectValue(old_data[index])) {
          ...
          if (existing &&
            ...
            throw NError.normal('Duplicate index entry attempted', { id: object.getID(), type: object.getType(), field: index });
          }
        }
      }
    }));
