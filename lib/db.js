...
class DB {
  ...
  static async modifyObject(viewer, object) {
        ...
        if (index in data) {
          if (Array.isArray(data[index])) {
            ...
            if (old_ii.length !== data[index].length || old_ii.filter(o => !data[index].includes(o)).length > 0) {
              await this.deleteIndex(master, index);
    ...
    await Promise.all((Constants.getObject(object.getType())['time_index'] || []).map(async index => {
  ...
  static async insertTimeIndex(object, key) {
    ...
    var value = null;
    try {
      value = new Date(key.startsWith('object.') ? (object.object[key.split('.')[1]] || Date.now()) : data[key]);
