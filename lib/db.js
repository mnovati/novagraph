...
async function checkEdgeCreate(that, viewer, edge) {
  ...
  if (edge.getType() === Constants.COGNITO_EDGE) {
    ...
    if (!object || Constants.getObject(object.getType()).root_id !== edge.getFromID()) {
      throw new Error('Cannot make a root edge where the from id is not a root id');
    }
  } else {
    ...
    if (!objects[0] || (config.from_type.length > 0 && !config.from_type.includes(objects[0].getType()))) {
      throw new Error('Invalid from type, or from object cannot be loaded for edge of type: ' + edge.getType());
    }
    if (!objects[1] || (config.to_type.length > 0 && !config.to_type.includes(objects[1].getType()))) {
      throw new Error('Invalid to type, or to object cannot be loaded for edge of type: ' + edge.getType());
    }
  }
...
function validateAndThrowObjectPieces(id, type, data) {
  if (!uuidValidate(id)) {
    throw new Error('Invalid id provided, expecting a uuid');
  }
  if (!Number.isInteger(type) || type < 0 || type >= Constants.MAX_OBJECT) {
    throw new Error('Invalid or unknown object type provided');
  }
  if (!data) {
    throw new Error('Missing or null data provided');
  }
  if (data.constructor !== Object) {
    throw new Error('Format of data was unexpected, json required');
  }
  if (Object.keys(data).length === 0) {
    throw new Error('Cannot mutate an object using an empty data object');
  }
}
...
class DB {
  ...
  static async createObject(viewer, type, data) {
    ...
    await Promise.all((Constants.getObject(type).unique_index || []).map(async index => {
      if (index in data) {
        ...
        if (existing && existing.length > 0) {
          throw new Error('Duplicate index entry attempted for type ' + type + ', field ' + index);
        }
      }
    }));
    ...
    await this.updateTextIndex(temp_object, null);
  ...
  static async modifyObject(viewer, object) {
    ...
    await Promise.all(Object.keys(data).map(async key => {
      var can = await old_object.filterField(key);
      if (!can) {
        delete data[key];
      }
    }));
    ...
    await Promise.all((Constants.getObject(object.getType()).unique_index || []).map(async index => {
      if (index in data) {
        ...
        if (existing &&
          ...
          throw new Error('Duplicate index entry attempted for type ' + object.getType() + ', field ' + index);
        }
      }
    }));
    ...
    await Promise.all(['index', 'unique_index'].map(async field => {
      await Promise.all((Constants.getObject(object.getType())[field] || []).map(async index => {
        if (index in data && stringifyObjectValue(data[index]) !== stringifyObjectValue(old_data[index])) {
    ...
    await this.updateTextIndex(object, old_object);
  ...
  static async setObjectStatus(viewer, object, new_status) {
    ...
    if (new_status === Constants.Status.DELETED) {
      await Promise.all(['index', 'unique_index'].map(async field => {
        await Promise.all((Constants.getObject(object.getType())[field] || []).map(async index => {
          if (index in data) {
