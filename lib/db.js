...
class DB {
  ...
  async getObject(viewer, id) {
    ...
    var object = this.Constants.getObjectInstance(this, viewer, result);
    if (!viewer.isReadAll()) {
      ...
      if (can_see) {
        ...
        result.data = data;
        object = this.Constants.getObjectInstance(this, viewer, result);
      } else {
        object = null;
      }
    }
  ...
  async createObject(viewer, type, data) {
    ...
    var temp_object = this.Constants.getObjectInstance(this, viewer, { id: uuid, type: type, data: data });
    if (!viewer.isWriteAll()) {
      ...
      temp_object = this.Constants.getObjectInstance(this, viewer, { id: uuid, type: type, data: data });
    }
    ...
    if (root_id) {
      ...
      await this.createEdge(
        write_all,
        this.Constants.getEdgeInstance(this, write_all, {from_id: root_id, to_id: uuid, type: this.Constants.ROOT_EDGE})
      );
    }
  ...
  async getSingleEdge(viewer, from_id, type, to_id) {
    ...
    result.type = parseInt(result.type);
    var edge = this.Constants.getEdgeInstance(this, viewer, result);
    if (!viewer.isReadAll()) {
  ...
  async getEdge(viewer, id, type) {
    ...
    for(var ii = 0; ii < results.length; ii++) {
      ...
      handles.push((async () => {
        var edge = this.Constants.getEdgeInstance(this, viewer, results[ii]);
        if (viewer.isReadAll()) {
          return edge;
        }
        var can_see = await edge.canSee();
  ...
  async createEdge(viewer, edge) {
    ...
    if (reverse_edge_type !== null) {
      ...
      var r_edge = this.Constants.getEdgeInstance(this, viewer, {
