...
async function checkEdgeCreate(that, viewer, edge) {
  ...
  var all_viewer = new AllReadViewer(0);
  var existing = await this.getSingleEdge(all_viewer, edge.getFromID(), edge.getType(), edge.getToID());
  if (existing) {
...
class DB {
  ...
  static async modifyObject(viewer, object) {
    ...
    if (old_object.getType() === 0 && old_object.getData().cognito_uuid) {
      data.cognito_uuid = old_object.getData().cognito_uuid;
    }
