...
async function checkObjectTypes(viewer, id, type, data) {
  var config = Constants.getObject(type);
...
class DB {
  ...
  static async createObject(viewer, type, data) {
    ...
    data = await checkObjectTypes(viewer, uuid, type, data);
  ...
  static async modifyObjectData(viewer, id, data_in) {
    ...
    validateAndThrowObjectPieces(id, master.getType(), data);
    ...
    data = await checkObjectTypes(viewer, id, master.getType(), data);
    if (!viewer.isWriteAll()) {
      await Promise.all(Object.keys(data).map(async key => {
    ...
    // check duplicated unique indexes
    await Promise.all((Constants.getObject(master.getType()).unique_index || []).map(async index => {
      if (index in data) {
    ...
    await Promise.all(['index', 'unique_index'].map(async field => {
      await Promise.all((Constants.getObject(master.getType())[field] || []).map(async index => {
        if (index in data) {
          if (Array.isArray(data[index])) {
            var old_ii = old_data[index] || [];
            if (old_ii.length !== data[index].length || old_ii.filter(o => !data[index].includes(o)).length > 0) {
              await this.deleteIndex(id, type, old_data, index);
              await this.insertIndex(id, type, data, index, field === 'unique_index');
            }
    ...
    await Promise.all((Constants.getObject(master.getType())['time_index'] || []).map(async index => {
      if ((index in data) && data[index] !== old_data[index]) {
        await this.insertTimeIndex(id, type, data, additional, index);
      } else if (index === 'object.time_updated') {
        await this.insertTimeIndex(id, type, data, additional, index);
      }
    }));

    var geo_index = Constants.getObject(master.getType()).geo_index;
  ...
  static async insertIndex(id, type, data, key, is_unique) {
  ...
  static async deleteIndex(id, type, data, key) {
    var starting = data[key];
    var values = [];
    if (Array.isArray(starting)) {
      starting.forEach(v => values.push(v));
  ...
  static async insertGeoIndex(id, type, data, key) {
  ...
  static async deleteGeoIndex(id) {
    var [result, _] = await this.execute(`
  ...
  static async insertTimeIndex(id, type, data, additional, key) {
    var value = null;
    try {
  ...
  static async deleteTimeIndex(id) {
    var [result, _] = await this.execute(`
