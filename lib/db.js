    ...
    await Promise.all((Constants.getObject(type).unique_index || []).map(async index => {
      if (index in data) {
        ...
        if (existing && existing.length > 0) {
          throw NError.normal('Duplicate index entry attempted', { type: type, field: index });
        }
      }
    }));
    ...
    await Promise.all((Constants.getObject(object.getType()).unique_index || []).map(async index => {
      if (index in data) {
        ...
        if (existing &&
          ...
          throw NError.normal('Duplicate index entry attempted', { type: object.getType(), field: index });
        }
      }
    }));
