...
class DB {
    ...
    if (reverse_edge_type !== null) {
      ...
      var [result, _] = await this.execute(
        'UPDATE edges SET data=? WHERE ' +
          '(from_id=uuid2bin(?) AND to_id=uuid2bin(?) AND type=?) OR ' +
          '(from_id=uuid2bin(?) AND to_id=uuid2bin(?) AND type=?);',
        [edge.getData(),
         edge.getFromID(), edge.getToID(), edge.getType(),
         r_edge.getFromID(), r_edge.getToID(), r_edge.getType()]
      );
      return result.affectedRows === 2;
    } else {
      await checkEdgeModify(viewer, edge);
      var [result, _] = await this.execute(
        'UPDATE edges SET data=? WHERE from_id=uuid2bin(?) AND to_id=uuid2bin(?) AND type=?;',
        [edge.getData(), edge.getFromID(), edge.getToID(), edge.getType()]
      );
      return result.affectedRows === 1;
    }
  }
  ...
  static async deleteEdge(viewer, edge) {
    ...
    if (reverse_edge_type !== null) {
      ...
      await checkEdgeModify(viewer, edge);
      var [result, _] = await this.execute(
        'UPDATE edges SET status=? WHERE from_id=uuid2bin(?) AND to_id=uuid2bin(?) AND type=?;',
        [Constants.Status.DELETED, edge.getFromID(), edge.getToID(), edge.getType()]
      );
      return result.affectedRows === 1;
    }
  }
