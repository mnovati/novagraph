...
class DB {
    ...
    if (indices) {
      ...
      for (var key in data) {
        if (indices.includes(key)) {
          handles.push(this.insertIndex(temp_object, key));
        }
      }
      await Promise.all(handles);
    }

    var geo_index = Constants.Objects[type]['geo_index'];
    if (geo_index) {
      for (var key in data) {
        if (geo_index === key) {
          await this.insertGeoIndex(temp_object, key);
        }
      }
    }
  ...
  static async modifyObject(viewer, object) {
    ...
    if (indices) {
      ...
      for (var key in data) {
        if (indices.includes(key)) {
          handles.push(this.deleteIndex(old_object, key));
        }
      }
      await Promise.all(handles);
      handles = [];
      for (var key in data) {
        if (indices.includes(key)) {
          handles.push(this.insertIndex(object, key));
        }
      }
      await Promise.all(handles);
    }
    ...
    if (geo_index) {
      for (var key in data) {
        if (geo_index === key) {
          ...
          await this.insertGeoIndex(object, key);
        }
      }
    }
  ...
  static async getSingleEdge(viewer, from_id, type, to_id) {
    ...
    if (viewer.existsCache(from_id+':'+type+':'+to_id)) {
      return viewer.fromCache(from_id+':'+type+':'+to_id);
    }
    var [rows, _] = await this.execute(
    ...
    viewer.setCache(from_id+':'+type+':'+to_id, edge);
    return edge;
  }
  ...
  static async getEdge(viewer, id, type) {
    ...
    if (viewer.existsCache(id+':'+type)) {
      return viewer.fromCache(id+':'+type);
    }
    var [rows, _] = await this.execute(
      'SELECT bin2uuid(from_id) as from_id, type, bin2uuid(to_id) as to_id, data, time_created, time_updated '+
      'FROM edges '+
  ...
  static async quickDeleteAllEdges(viewer, id) {
    if (!viewer.isWriteAll()) {
    ...
    var [result, _] = await this.execute(
      ...
      [Constants.Status.DELETED, id, id, Constants.Status.VISIBLE]
    );
  }
  ...
  static async lookupGeoIndex(point, types, distance) {
    ...
    var value = 'POINT ('+point.lat+' '+point.lng+')';
    var [rows, _] = await this.execute(
      'SELECT bin2uuid(`id`) as id '+
      'FROM geoindices '+
      'WHERE `type` in (?) AND st_distance_sphere(shape, st_geomfromtext(?, 1)) <= ?;',
      [types.join(','), value, distance]
    );
    if (!rows) {
      return null;
    }
  ...
  static async insertGeoIndex(object, key) {
    ...
    var [result, _] = await this.execute(
      'INSERT IGNORE INTO geoindices (`id`, `type`, `shape`) VALUES (uuid2bin(?), ?, st_geomfromtext(?, 1));',
      [object.getID(), object.getType(), 'POINT ('+data[key].lat+' '+data[key].lng+')']
    );
    return true;
  }

  static async deleteGeoIndex(object, key) {
    var data = await object.getData();
    var value = stringifyObjectValue(data[key]);
    if (!value) {
      return false;
