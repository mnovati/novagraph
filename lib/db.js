...
async function checkObjectTypes(Constants, viewer, id, type, data) {
...
class DB {
  ...
  async createObject(viewer, type, data) {
    ...
    data = await checkObjectTypes(this.Constants, viewer, uuid, type, data);
  ...
  async modifyObjectData(viewer, id, data_in) {
    ...
    data = await checkObjectTypes(this.Constants, viewer, id, master.getType(), data);
    if (!viewer.isWriteAll()) {
      await Promise.all(Object.keys(data).map(async key => {
