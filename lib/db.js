...
class DB {
  ...
  static async createObject(viewer, type, data) {
    ...
    await this.updateTextIndex(temp_object);
  ...
  static async modifyObject(viewer, object) {
    ...
    await this.updateTextIndex(object);

    return true;
  }

  static async deleteObjectAndEdges(viewer, id, type) {
    var object = await DB.getObject(viewer, id);
    if (!object) {
      throw new Error('Error loading object to delete with id: ' + id);
    }
    if (object.getType() !== type) {
      throw new Error('Object type does not match requested type');
    }
    var result = await this.setObjectStatus(viewer, object, Constants.Status.DELETED);
    if (!result) {
      throw new Error('Error deleting object');
    }
    await this.quickChangeStatusAllEdges(
  ...
  static async deleteSingleEdge(viewer, from_id, from_type, to_id, edge_type) {
    if (!from_id) {
      throw new Error('Missing source id for edge deletion');
    }
    if (!to_id) {
      NovaError.throwError('Missing destination id for edge modification');
    }
    var object = await DB.getObject(viewer, from_id);
    if (object.getType() !== from_type) {
      throw new Error('Object type does not match requested type');
    }
    var existing = await DB.getSingleEdge(viewer, from_id, edge_type, to_id);
    if (!existing) {
      throw new Error('Cannot cannot be loaded or may not exists');
    }
    await DB.deleteEdge(viewer, existing);
  }
  ...
  static async lookupTextIndex(index_type, text) {
    ...
    var [rows, _] = await this.execute(
      'SELECT bin2uuid(`id`) as id MATCH(`data`) AGAINST(? IN NATURAL LANGUAGE MODE) as score FROM ftsindices '+
      'WHERE `type`=? and MATCH(`data`) AGAINST(? IN NATURAL LANGUAGE MODE) > 0 '+
      'ORDER BY score DESC;',
      [value, index_type, value]
    );
    if (!rows) {
      return null;
    }
