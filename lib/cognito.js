  ...
  async createUserAccountFromProfile(token, profile_id, data) {
    ...
    try {
      ...
      if (uuid) {
        ...
        var result = await this._DB.modifyObjectData(new WriteAllViewer(0), profile_id, 0, old_data);
        if (result) {
    ...
    try {
      ...
      if (uuid) {
        ...
        if (object_id) {
          ...
          await this._DB.modifyObject(viewer, object_id, 0, { creator_id: object_id });
          return object_id;
        }
      }
      throw NError.normal('Error creating object for Cognito uuid', { uuid: uuid });
