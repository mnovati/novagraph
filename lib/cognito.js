  ...
  async user(token) {
    return new Promise((resolve, reject) => {
      ...
      if (temp.payload.iss === 'https://cognito-identity.amazonaws.com') {
        ...
        if (temp.payload.iss.split('/').pop() !== _config.cognitoUserPoolId) {
          throw NError.normal('Token does not match the expected source');
        }
        cert = `https://cognito-idp.${_config.region}.amazonaws.com/${_config.cognitoUserPoolId}/.well-known/jwks.json`;
      }
    ...
    try {
      ...
      if (uuid) {
        ...
        if (response.username) {
          data.cognito_username = response.username.toLowerCase();
        }
